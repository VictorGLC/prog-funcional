...
pub fn main(jogos: List(String)) -> Result(List(String), Erros) {
  case verifica_lista_jogos(jogos) {
    Error(erro) -> Error(erro)
    Ok(jogos) ->
      case
        ordena_desempenhos(calcula_lista_desempenhos(
          jogos,
          encontra_times(jogos),
        ))
      {
        desempenhos -> Ok(converte_desempenhos_em_string(desempenhos))
      }
  }
}
...
pub fn main_examples() {
  ...
  check.eq(
    ...
    Ok([
      "Atletico-MG 6 2 4", "Vasco 4 1 1", "Botafogo 1 0 -3", "Cruzeiro 0 0 -2",
    ]),
  )
  ...
  check.eq(
    ...
    Ok(["Fortaleza 7 2 4", "Bahia 4 1 2", "Cuiaba 0 0 -1", "Ceara 0 0 -5"]),
  )
...
pub fn verifica_lista_jogos(jogos: List(String)) -> Result(List(Jogo), Erros) {
  case jogos {
    [] -> Ok([])
    [primeiro, ..resto] ->
      case verifica_jogo(primeiro), verifica_lista_jogos(resto) {
        Error(erro), _ -> Error(erro)
        _, Error(erro) -> Error(erro)
        Ok(jogo), Ok(resto) -> Ok([jogo, ..resto])
      }
  }
}
...
// Recebe lista de *jogos* e produz uma lista de strings com todos os times que participaram dos jogos, sem repetição de times.
pub fn encontra_times(jogos: List(Jogo)) -> List(String) {
  case jogos {
    [] -> []
    [primeiro, ..resto] ->
      case
        contem(encontra_times(resto), primeiro.anfitriao),
        contem(encontra_times(resto), primeiro.visitante)
      {
        False, False -> [
          primeiro.anfitriao,
          primeiro.visitante,
          ..encontra_times(resto)
        ]
        True, False -> [primeiro.visitante, ..encontra_times(resto)]
        False, True -> [primeiro.anfitriao, ..encontra_times(resto)]
        True, True -> encontra_times(resto)
      }
  }
}
...
pub fn encontra_times_examples() {
  check.eq(
    ...
    ["Flamengo", "Cuiaba", "Gremio", "Bragantino", "Criciuma"],
  )
  check.eq(
    encontra_times([
  ...
  check.eq(
    ...
    ["Vasco", "Santos", "Bota-Fogo", "Atletico-MG"],
  )
  check.eq(
    encontra_times([
  ...
  check.eq(
    ...
    ["Vasco", "Palmeiras", "Santos", "Bota-Fogo", "Atletico-MG"],
  )
  check.eq(encontra_times([]), [])
}

// função auxiliar que recebe *lst* e produz True ou False se *str* esta em *lst*
pub fn contem(lst: List(String), str: String) -> Bool {
  case lst {
    [] -> False
    [primeiro, ..resto] -> primeiro == str || contem(resto, str)
  }
}

pub fn contem_examples() {
  check.eq(contem([], "palmeiras"), False)
  check.eq(contem(["vasco"], "palmeiras"), False)
  check.eq(contem(["palmeiras", "flamengo", "corinthians"], "palmeiras"), True)
  check.eq(
    contem(["palmeiras", "flamengo", "corinthians"], "corinthians"),
    True,
  )
  check.eq(contem(["palmeiras", "flamengo", "corinthians"], "flamengo"), True)
  check.eq(contem(["flamengo", "goias", "bragantino"], "fluminense"), False)
}
  ...
  case jogos {
    ...
    [primeiro, ..resto] ->
      case primeiro.anfitriao == time || primeiro.visitante == time {
        True -> [primeiro, ..filtra_jogos_por_time(resto, time)]
        False -> filtra_jogos_por_time(resto, time)
      }
  }
}
...
pub fn calcula_lista_desempenhos(
  ...
  case jogos, times {
    ...
    [], [_, ..] -> []
    [_, ..], [] -> []
    [_, ..], [primeiro_time, ..resto_time] ->
      case filtra_jogos_por_time(jogos, primeiro_time) {
        jogos_filtrados -> [
          calcula_desempenho(
            jogos_filtrados,
            primeiro_time,
            Desempenho(primeiro_time, 0, 0, 0),
          ),
          ..calcula_lista_desempenhos(jogos, resto_time)
        ]
      }
  }
}
      ...
      case primeiro.anfitriao == time, primeiro.visitante == time {
        True, False ->
          calcula_desempenho(
            ...
            time,
            desempenho_time(
              primeiro.gols_anfitriao,
              primeiro.gols_visitante,
              desempenho,
            ),
          )
        ...
        _, _ -> calcula_desempenho(resto, time, desempenho)
      }
  }
}
...
// recebe lista de *desempenhos* e produz lista de *desempenhos* ordenada nos criterios de classificação do brasileirão
// caso haja empate, o desempate é feito por ordem hierarquica segundo os criterios: vitórias, saldo de gols e pela ordem alfabética.
pub fn ordena_desempenhos(desempenhos: List(Desempenho)) -> List(Desempenho) {
  case desempenhos {
    [] -> []
    [primeiro, ..resto] ->
      insere_desempenho_ordenado(primeiro, ordena_desempenhos(resto))
  }
}
...
// Recebe *novo_desempenho* e lista de *desempenhos*, deve produzir uma nova lista de desempenhos com *novo_desempenho* sendo inserido ordenado.
pub fn insere_desempenho_ordenado(
  ...
  case desempenhos {
    [] -> [novo_desempenho]
    [primeiro, ..resto] ->
      case compara_desempenhos(novo_desempenho, primeiro) == novo_desempenho {
        True -> [novo_desempenho, ..desempenhos]
        False -> [
          primeiro,
          ..insere_desempenho_ordenado(novo_desempenho, resto)
        ]
      }
  }
}
...
pub fn insere_desempenho_ordenado_examples() {
  check.eq(
    insere_desempenho_ordenado(Desempenho("Palmeiras", 3, 1, 1), [
      Desempenho("Santos", 6, 2, 4),
      Desempenho("Bota-Fogo", 3, 1, -1),
      Desempenho("Atletico-MG", 1, 0, -2),
      Desempenho("Vasco", 1, 0, -2),
  ...
  check.eq(
    insere_desempenho_ordenado(Desempenho("Vasco", 1, 0, -2), [
      Desempenho("Santos", 6, 2, 4),
      Desempenho("Bota-Fogo", 3, 1, -1),
      Desempenho("Atletico-MG", 1, 0, -2),
    ]),
  ...
  check.eq(
    insere_desempenho_ordenado(Desempenho("Santos", 6, 2, 4), [
      Desempenho("Bota-Fogo", 3, 1, -1),
      Desempenho("Atletico-MG", 1, 0, -2),
    ]),
    [
...
pub fn converte_desempenhos_em_string(
  ...
  case desempenhos {
    ...
    [primeiro, ..resto] -> [
      ...
      ..converte_desempenhos_em_string(resto)
    ]
  }
}
...
pub fn converte_desempenhos_em_string_examples() {
  check.eq(
    ...
    [
      "Palmeiras 3 1 1", "Vasco 1 0 -2", "Santos 6 2 4", "Bota-Fogo 3 1 -1",
      "Atletico-MG 1 0 -2",
    ],
  )
}
